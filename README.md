...
#### 20210524(월) 작업.
- 폰트 어썸(Font Awesome 아이콘 웹폰트) : 아이콘을 확대해도 깨지지 않음(백터 방식)
- 포토샵 디자인으로 아이콘을 만들면 확대 시 깨짐()
- 부트스트랩 레이아웃 용어: 화면을 12개의 컬럼으로 분리해서 크기를 반응형으로 제작.
- row(가로줄), col(세로칸), col-md-12(화면 가로 크기를 12로 지정 = 100%)
